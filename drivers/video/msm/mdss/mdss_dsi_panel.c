...
static struct {
    struct dsi_panel_cmds cmd_prefix;
    struct dsi_panel_cmds cmd_postfix;
    struct dsi_panel_cmds ce_on_cmd;
    struct dsi_panel_cmds ce_off_cmd;
...
static int mdss_dsi_update_cabc_level(struct mdss_dsi_ctrl_pdata *ctrl)
{
	...
	if (pinfo->ce_available)
		len += pinfo->ce_enabled ? cabc_data.ce_on_cmd.blen : cabc_data.ce_off_cmd.blen;
	...
	len += cabc_data.cmd_prefix.blen + cabc_data.cmd_postfix.blen;
	...
	if (pinfo->ce_available) {
		struct dsi_panel_cmds *ce_cmd = pinfo->ce_enabled
			? &cabc_data.ce_on_cmd : &cabc_data.ce_off_cmd;
		memcpy(cmd_buf + dlen, ce_cmd->buf, ce_cmd->blen);
		dlen += ce_cmd->blen;
	}

	memcpy(cmd_buf + dlen, cabc_data.cmd_prefix.buf, cabc_data.cmd_prefix.blen);
	dlen += cabc_data.cmd_prefix.blen;
	// The CABC command parameter is the last value in the sequence
	cmd_buf[dlen - 1] = cabc_value;

	memcpy(cmd_buf + dlen, cabc_data.cmd_postfix.buf, cabc_data.cmd_postfix.blen);
	dlen += cabc_data.cmd_postfix.blen;

	// Parse the command and send it
	ret = parse_dsi_cmds(&dsi_cmds, cmd_buf, len);
	if (ret == 0) {
...
#ifdef VENDOR_EDIT
/* Xiaori.Yuan@Mobile Phone Software Dept.Driver, 2014/02/17  Add for set cabc */
	rc = mdss_dsi_parse_dcs_cmds(np, &cabc_data.cmd_prefix,
		"qcom,mdss-dsi-cabc-cmd", "qcom,mdss-dsi-off-command-state");
	if (rc == 0)
		rc = mdss_dsi_parse_dcs_cmds(np, &cabc_data.cmd_postfix,
			"qcom,mdss-dsi-cabc-post-cmd", "qcom,mdss-dsi-off-command-state");
	pinfo->cabc_available = rc == 0;
	...
	rc = mdss_dsi_parse_dcs_cmds(np, &cabc_data.ce_on_cmd,
		"qcom,mdss-dsi-color-enhance-on", "qcom,mdss-dsi-off-command-state");
	if (rc == 0)
		rc = mdss_dsi_parse_dcs_cmds(np, &cabc_data.ce_off_cmd,
			"qcom,mdss-dsi-color-enhance-off", "qcom,mdss-dsi-off-command-state");
	pinfo->ce_available = rc == 0;
